...
Rcpp::List btf_selected_cov_cpp(
  ...
  if (evol_sigma_t2.n_rows != T ||
      evol_sigma_t2.n_cols != p)
    Rcpp::stop(
      "evol_sigma_t2 must be nrow(X) x ncol(X)."
    );

  if (!X.is_finite() ||
      !obs_sigma_t2.is_finite() ||
      !evol_sigma_t2.is_finite())
  ...
  for (arma::uword t = 0; t < T; ++t) {
    ...
    if (obs_sigma_t2(t) <= 0.0)
      Rcpp::stop(
        "obs_sigma_t2 must be strictly positive."
      );

    for (arma::uword j = 0; j < p; ++j) {
      if (evol_sigma_t2(t,j) <= 0.0)
        Rcpp::stop(
          "evol_sigma_t2 must be strictly positive."
        );
    }
  }

  // G_t = inverse forward Schur complement.
  // P_t = desired diagonal block of Qpost^{-1}.
  arma::cube G(p, p, T, arma::fill::zeros);
  arma::cube P(p, p, T, arma::fill::zeros);

  //----------------------------------------------------------
  // Forward block factorization
  //----------------------------------------------------------
  ...
  for (arma::uword t = 0; t < T; ++t) {
    ...
    // Observation precision block:
    // x_t x_t^T / sigma_t^2
    arma::mat F =
      (xt * xt.t()) / obs_sigma_t2(t);

    // Evolution precision diagonal block.
    for (arma::uword j = 0; j < p; ++j) {

      F(j,j) +=
        1.0 / evol_sigma_t2(t,j);

      if (t + 1 < T)
        F(j,j) +=
          1.0 / evol_sigma_t2(t + 1,j);
    }

    if (t > 0) {

      // Off-diagonal precision block C_{t-1}
      // is diagonal:
      // -diag(1 / evol_sigma_t2[t, ])
      arma::vec c(p);
    ...
    bool ok =
      arma::inv_sympd(invF, F);

    if (!ok)
      Rcpp::stop(
        "Selected-inverse recursion failed at time %d.",
        static_cast<int>(t + 1)
      );
  ...
  //----------------------------------------------------------
  // Backward selected-inverse recursion
  //----------------------------------------------------------
  ...
  return Rcpp::List::create(
    Rcpp::Named("cov") = P,
    Rcpp::Named("var") = post_var
  );
}
',
cacheDir = rcpp_cache_dir,
rebuild = FALSE,
showOutput = FALSE,
verbose = FALSE,
echo = FALSE
...
sampleBTF_reg = function (y, X, obs_sigma_t2, evol_sigma_t2, XtX, D = 1, chol0 = NULL, return_mean = FALSE, return_var = FALSE) 
{
  ...
  if (return_var){
    ...
    if (D != 1)
      stop(
        "The contemporaneous posterior covariance calculation requires D = 1."
      )
    
    selected_cov <- btf_selected_cov_cpp(
      X = X,
      obs_sigma_t2 = as.numeric(obs_sigma_t2),
      evol_sigma_t2 = evol_sigma_t2
    )
    post_cov_blocks <- aperm(
      selected_cov$cov,
      c(3, 1, 2)
    )
...
### nsave = number of saved MCMC samples, nburn = burnin period of the MCMC sampler, nskip = level of thining for the MCMC sampler###
### mcmc_params = list of parameters to savem, and the function returns MCMC samples of these quantities of interest ###
{
  computeDIC = FALSE
  ...
  if (!is.null(checkpoint_state)) {
    ...
    factor_cov = factorstochvol::covmat(
      factor_model,
      timepoints = "all"
    )
  ...
  expected_factor_cov_dim =
    c(5, 5, nsave, nrow(y))
  
  if (!identical(
    as.integer(dim(factor_cov)),
    as.integer(expected_factor_cov_dim)
  )) {
    stop(
      paste(
        "Unexpected factor_cov dimensions:",
        paste(dim(factor_cov), collapse = " x "),
        "- expected",
  ...
  for(nsi in iterations_to_run){
    ...
    repeat {
      ...
      new_svParams = tryCatch(
        fsvsample(
          something,
          factors = number_of_latent_factors,
          draws = 1,
          thin = 1,
          burnin = 0,
          keeptime = "all",
    ...
    if (need_score_moments){
      ...
      for(i in 1:T){
        ...
        # N x 5 posterior mean beta matrix
        mean_B_t = meanbeta[,i,2:6]
        
        # 5 x 5 FF factor covariance
        H_t = factor_cov[,,factor_draw,i]
        
        # Mean-beta contribution
        systematic_cov =
          mean_B_t %*%
          H_t %*%
          t(mean_B_t)
        
        # Posterior uncertainty in all five betas
        for(j in 1:N){
      ...
      if (skipcount > nskip) {
        ...
        if (!is.null(checkpoint_file) &&
          ...
          checkpoint = list(
            ...
            evolParams = evolParams,
            evolParams0 = evolParams0,
...
FF5 <- as.matrix(
  daily_data[, c("Mkt.RF", "SMB", "HML", "RMW", "CMA")]
)
...
market_file <- file.path(
  data_dir,
  "market_data_2019_2023.rds"
)
...
daily_tech_excess_returns <- matrix(
  nrow = nrow(daily_tech_data[[1]]) - 1,
  ncol = length(tech_stocks)
)

for (i in seq_along(tech_stocks)) {
  daily_tech_excess_returns[, i] <-
    100 * diff(log(daily_tech_data[[i]][, 6]))[-1] -
    daily_data$RF
}


daily_diversify_excess_returns <- matrix(
  nrow = nrow(daily_diversify_data[[1]]) - 1,
  ncol = length(diversify_stocks)
)

for (i in seq_along(diversify_stocks)) {
  daily_diversify_excess_returns[, i] <-
    100 * diff(log(daily_diversify_data[[i]][, 6]))[-1] -
    daily_data$RF
}
...
dspmfsv_daily_tech <- DSP_MFSV(
  y = daily_tech_excess_returns,
  X = design_matrix,
  nsave = 3000,
  nburn = 1500,
...
saveRDS(
  dspmfsv_daily_tech,
  file.path(output_dir, "dspmfsv_daily_tech.rds"),
  compress = FALSE
)
...
dspmfsv_daily_diversifys <- DSP_MFSV(
  y = daily_diversify_excess_returns,
  X = design_matrix,
  nsave = 3000,
  nburn = 1500,
  number_of_latent_factors = 3,
...
## Save immediately after completion
saveRDS(
  dspmfsv_daily_diversifys,
  file.path(output_dir, "dspmfsv_daily_diversified.rds"),
  compress = FALSE
)

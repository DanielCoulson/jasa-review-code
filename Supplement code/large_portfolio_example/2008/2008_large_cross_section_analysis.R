############################################################
## 1. Locate and load all 2008 large-cross-section results
############################################################

base_dir <- if (
  file.exists("large_cross_section_cluster_input.rds")
) {
  "."
} else {
  "large_cross_section_2008_results"
}
...
stopifnot(
  all(dim(A) == c(1000, 50)),
  all(dim(dsp_rep$covariance_mean) == c(50, 50, 1006)),
  length(residual_fits) == 50,
  length(dates) == 1006
)

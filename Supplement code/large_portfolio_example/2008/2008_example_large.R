...
input_1000 <- readRDS(
  "large_cross_section_1000_input.rds"
)
...
############################################################
## STEP 7: Freeze K = 50 and extract medoids/clusters
############################################################
...
stopifnot(K_star == 50L)

pam_star <- pam_fits[[best_idx]]

# Indices of the 50 medoid stocks within the 1000-stock universe
...
representative_permnos <-
  universe_1000$PERMNO[medoid_idx]

# Corresponding December-2005 tickers
representative_tickers <-
  universe_1000$Ticker[medoid_idx]
...
colnames(representative_excess_returns) <-
  as.character(representative_permnos)

stopifnot(
  all(dim(representative_excess_returns) == c(1006, 50))
)
...
############################################################
## STEP 8: Relate all 1000 stocks to the 50 representatives
############################################################

# Make absolutely sure medoids are ordered by cluster label
medoid_cluster <- as.integer(
  cluster_membership[medoid_idx]
)

stopifnot(
  length(unique(medoid_cluster)) == K_star,
  all(sort(medoid_cluster) == seq_len(K_star))
)

medoid_idx_by_cluster <- medoid_idx[
  match(seq_len(K_star), medoid_cluster)
]

stopifnot(
  all(
    cluster_membership[medoid_idx_by_cluster] ==
      seq_len(K_star)
  )
)

# Representative return matrix ordered by cluster
representative_excess_returns <-
  daily_1000_excess_returns[
    ,
...
representative_permnos <-
  universe_1000$PERMNO[medoid_idx_by_cluster]
...
fitted_from_reps <-
  representative_excess_returns %*% t(A)
...
E <- daily_1000_excess_returns -
  fitted_from_reps

# Representatives must have exactly zero residual
...
stopifnot(
  all(dim(E) == c(1006, 1000)),
  all(E[, medoid_idx_by_cluster] == 0)
)
...
############################################################
## STEP 9: Form the 50 residual blocks
############################################################
...
residual_block_indices <- lapply(
  ...
  function(g) {
    ...
    # Remove that cluster's medoid because its residual is zero
    setdiff(
      members,
...
stopifnot(
  length(residual_block_indices) == 50,
  sum(block_sizes) == 950
)
...
representatives_by_cluster <- data.frame(
  Cluster = seq_len(K_star),
  Index = medoid_idx_by_cluster,
  PERMNO = universe_1000$PERMNO[
    medoid_idx_by_cluster
  ],
  Ticker = universe_1000$Ticker[
    medoid_idx_by_cluster
  ],
  MthCap = universe_1000$MthCap[
    medoid_idx_by_cluster
  ]
)

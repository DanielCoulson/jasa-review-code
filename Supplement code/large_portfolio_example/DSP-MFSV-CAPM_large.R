...
### mcmc_params = list of parameters to save, and the function returns MCMC samples of these quantities of interest ###
{
  ...
  save_covariance_mean <- ("covariance_mean" %in% mcmc_params)
  ...
  if (save_covariance_mean) {
    post_covariance_sum <- array(0,dim = c(N, N, T))
  }
  ...
  for(nsi in 1:nstot){
    ...
    need_covariance_moments <- (save_scores || save_covariance_mean) && will_save_this_iter
    if (nsi %% 100 == 0) {
    ...
    for(i in 1:N){
      res = sampleBTF_reg(
        ...
        return_mean = need_covariance_moments,
        return_var = need_covariance_moments )
      beta[i, , ] <- res$beta
      if (need_covariance_moments){
    ...
    if (need_covariance_moments) {
      ...
      for (i in seq_len(T)) {
        covariance_i <- diag(varbeta[, i, 1]) + variances[nsi, i] *(diag(varbeta[, i, 2]) +tcrossprod(meanbeta[, i, 2])) +stuff[, , i]
        if (save_covariance_mean) {
          post_covariance_sum[, , i] <- post_covariance_sum[, , i] + covariance_i
        }
        if (save_scores) {
          scores[i] <-score(cov2cor(covariance_i))
        }
      }
    }
    if (nsi > nburn) {
  ...
  if (save_covariance_mean) {
    mcmc_output$covariance_mean <-post_covariance_sum / nsave
  }
  return(mcmc_output)
}

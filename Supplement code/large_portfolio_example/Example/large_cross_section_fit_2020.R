...
input <- readRDS(file.path(base_dir,"large_cross_section_cluster_input_2020.rds"))
...
if (task_id == 1L) {
  ...
  cat("Fitting 39-stock DSP-MFSV-CAPM\n")
  fit <- DSP_MFSV(y = Y_rep,X = X_rep,D = 1,number_of_latent_factors = 3,nsave = 3000,nburn = 1500,nskip = 4,
    mcmc_params = c("covariance_mean"))
  ...
  fit <- factorstochvol::fsvsample(Y_block,factors = factors_g,draws = 12000,
    thin = 4,burnin = 1500,keeptime = "all",quiet = TRUE)
  T_obs <- nrow(Y_block)

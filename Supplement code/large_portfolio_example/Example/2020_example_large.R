...
#### prepare data ####
setwd("~/Downloads")
input_1000 <- readRDS("large_cross_section_1000_input_2020.rds")
...
############################################################
## STEP 7: Freeze selected K and extract medoids
############################################################
...
representatives <- data.frame(Cluster = seq_len(K_star),
  Index = medoid_idx,PERMNO = representative_permnos,
  Ticker = representative_tickers,MthCap =universe_1000$MthCap[medoid_idx])
...
print(dim(representative_excess_returns))
saveRDS(list(K = K_star,pam = pam_star,medoid_idx = medoid_idx,
    representatives = representatives,cluster_membership =
      cluster_membership, representative_excess_returns =representative_excess_returns),
...
############################################################
representative_excess_returns <-daily_1000_excess_returns[,medoid_idx_by_cluster,
    drop = FALSE]
...
representatives_by_cluster <-data.frame(Cluster =seq_len(K_star),
    ...
    PERMNO =universe_1000$PERMNO[medoid_idx_by_cluster],
...
saveRDS(list(dates = ff_dates,market_excess_return =daily_data$Mkt.RF,
    ...
    residual_blocks =residual_blocks),"large_cross_section_cluster_input_2020.rds")

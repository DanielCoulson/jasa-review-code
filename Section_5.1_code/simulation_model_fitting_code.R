...
bootstrap_score_intervals <- function(Y,fit_fun,B = 200,block_length = 20,
    seed = seed,probs = c(0.025, 0.975)) {
...
fit_mfsv_scores <- function(Y,factors = 3,draws = 12000,thin = 4,
    burnin = 1500,quiet = TRUE,interval_k = 2) {
  fit <- factorstochvol::fsvsample(Y,factors = factors,draws = draws,
    thin = thin,burnin = burnin,keeptime = "all",quiet = quiet)
  TT <- nrow(Y)
...
fit_dsp_mfsv_scores <- function(Y,X,number_of_latent_factors = 3,
    nsave = 3000,nburn = 1500,nskip = 4,credMass = 0.95) {
  fit <- DSP_MFSV(y = Y, X = X,number_of_latent_factors = number_of_latent_factors,
    nsave = nsave,nburn = nburn,nskip = nskip,mcmc_params = c("scores"))
...
fit_ht_roll_scores_boot <- function(Y,window = 60,tau = 0.10,B = 200,
    block_length = 20,seed = seed) {
  bootstrap_score_intervals(Y = Y,
    fit_fun = function(Z) fit_ht_roll_scores(Z, window = window, tau = tau),
    B = B,block_length = block_length,seed = seed)
}
...
fit_loggle_scores <- function(Y, seed = seed){
  ...
  loggle_score <- fit_loggle_path(Y,h = h.opt,d = d.opt,lambda = lambda.opt)
  list(
    mean = loggle_score, 
    lower = rep(NA_real_, TT),
    upper = rep(NA_real_, TT),
...
fit_loggle_scores_boot <- function(Y, B=200, block_length = 20, seed = seed,
  ...
  for (b in seq_len(B)) {
    idx <- boot_indices[[b]]
    Yb <- Y[idx,,drop = FALSE]
    boot_scores[, b] <- fit_loggle_path(Yb,h = h.opt,d = d.opt,lambda = lambda.opt)
  }
...
fit_dcc <- function(Y,model = c("DCC", "aDCC"),distribution = c("mvnorm", "mvt")
) {
  ...
  spec <- rmgarch::dccspec(uspec = uspec,dccOrder = c(1, 1),model = model,
    distribution = distribution)
  rmgarch::dccfit(spec = spec,data = Y,fit.control = list(eval.se = FALSE))
}
...
fit_dcc_with_uncertainty <- function(Y,
  ...
  fit <- fit_dcc( Y,model = model,distribution = distribution)
  R_hat <- rmgarch::rcor(fit)
  TT <- dim(R_hat)[3]
  score_hat <- vapply(seq_len(TT),
...
fit_chol_sv_scores <- function(Y,lags = 1L,draws = 1500L,burnin = 1500L,
    thin = 1L,cholesky_U_prior = "HS",center_data = TRUE,quiet = TRUE,
    interval_k = 2) {
  ...
  TT_full <- nrow(Y_in)
  N <- ncol(Y_in)
  prior_sigma <- bayesianVARs::specify_prior_sigma(data = Y_in,type = "cholesky",
    cholesky_U_prior = cholesky_U_prior,cholesky_heteroscedastic = TRUE,
    quiet = TRUE)
  fit <- bayesianVARs::bvar(data = Y_in,lags = lags,draws = draws,burnin = burnin,
    thin = thin,prior_intercept = FALSE,prior_sigma = prior_sigma,sv_keep = "all",
    quiet = quiet)
...
fit_ew_roll_scores_boot <- function(Y,window = 60,lambda = 0.94,B = 200,
    block_length = 20,seed = seed) {
  bootstrap_score_intervals(Y = Y,
...
fit_lw_roll_scores_boot <- function(Y,window = 60,B = 200,block_length = 20,seed = seed) {
  bootstrap_score_intervals(Y = Y,
...
run_all_score_models <- function(sim,truth_target = c("asset", "eps", "joint"),
    mfsv_factors = 3,mfsv_draws = 12000,mfsv_thin = 4,mfsv_burnin = 1500,
    dsp_factors = 3,dsp_nsave = 3000,dsp_nburn = 1500,dsp_nskip = 4,
    roll_window = 60,ew_lambda = 0.94,B_nonbayes = 200,block_length = 20,
    bootstrap_seed = seed,credMass = 0.95,ht_tau = 0.10,cholsv_lags = 1,
    cholsv_draws = 12000,cholsv_burnin = 1500,cholsv_thin = 4,cholsv_u_prior = "HS") {
  truth_target <- match.arg(truth_target)
  Y <- sim$Y
  X <- sim$X
  TT <- nrow(Y)
  ...
  out <- list()
  out$truth <- data.frame(t = seq_len(TT),
    true_score = extract_true_scores(sim, truth_target = truth_target))
  
  out$methods <- list()
  
  message("Fitting MFSV...")
  out$methods$mfsv <- fit_mfsv_scores(Y = Y,factors = mfsv_factors,draws = mfsv_draws,
    thin = mfsv_thin,burnin = mfsv_burnin)
  
  message("Fitting Cholesky SV...")
  out$methods$chol_sv <- fit_chol_sv_scores(Y = Y,lags = cholsv_lags,
    draws = cholsv_draws,burnin = cholsv_burnin,thin = cholsv_thin,
    cholesky_U_prior = cholsv_u_prior)
  
  message("Fitting DSP_MFSV...")
  out$methods$dsp_mfsv <- fit_dsp_mfsv_scores(Y = Y,X = X,
    number_of_latent_factors = dsp_factors,nsave = dsp_nsave,nburn = dsp_nburn,
    nskip = dsp_nskip,credMass = credMass)
  
  message("Fitting exponentially weighted rolling estimator with bootstrap...")
  out$methods$ew_roll <- fit_ew_roll_scores_boot(Y = Y,
    window = roll_window,lambda = ew_lambda,B = B_nonbayes,
    block_length = block_length,seed = bootstrap_seed)
  
  message("Fitting rolling Ledoit-Wolf with bootstrap...")
  out$methods$roll_lw <- fit_lw_roll_scores_boot(Y = Y,
    window = roll_window,B = B_nonbayes,block_length = block_length,
    seed = bootstrap_seed)
  
  message("Fitting hard-thresholded rolling estimator with bootstrap...")
  out$methods$ht_roll <- fit_ht_roll_scores_boot(
    Y = Y,window = roll_window,tau = ht_tau,B = B_nonbayes,
    block_length = block_length,seed = bootstrap_seed)
  
  message("Fitting LOGGLE with bootstrap...")
  out$methods$loggle <- fit_loggle_scores_boot(Y = Y,B = B_nonbayes,
    block_length = block_length,seed = bootstrap_seed)
  
  message("Fitting Gaussian DCC with Fresoli-Ruiz bootstrap uncertainty...")
  out$methods$dcc_norm <- fit_dcc_with_uncertainty(Y = Y,
    model = "DCC",distribution = "mvnorm",B = B_nonbayes,
    seed = bootstrap_seed,probs = c((1 - credMass) / 2, 1 - (1 - credMass) / 2))
  
  message("Fitting t-DCC with Fresoli-Ruiz bootstrap uncertainty...")
  out$methods$dcc_t <- fit_dcc_with_uncertainty(Y = Y,model = "DCC",
    distribution = "mvt",B = B_nonbayes,seed = bootstrap_seed + 1L,
    probs = c((1 - credMass) / 2, 1 - (1 - credMass) / 2))
  
  message("Fitting asymmetric DCC with Fresoli-Ruiz bootstrap uncertainty...")
  out$methods$adcc <- fit_dcc_with_uncertainty(Y = Y,
    model = "aDCC",distribution = "mvnorm",B = B_nonbayes,seed = bootstrap_seed + 2L,
    probs = c((1 - credMass) / 2, 1 - (1 - credMass) / 2))
  message("Done.")
  out
}
